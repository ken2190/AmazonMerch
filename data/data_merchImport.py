...
def getProducts(my_mongo,query={},limit=0):
    query["status"] = "New"
    myclient = my_mongo.create_mongo()
    data = my_mongo.find(myclient,"amz_merch","Product",query,{},limit=limit)
    my_mongo.close_mongo(myclient)
...
def getIdByNameAccount(my_mongo,name):
    myclient = my_mongo.create_mongo()
    data = my_mongo.find(myclient,"amz_merch","Account",{"name":name},{"name":1,"id":1},limit=1)
    my_mongo.close_mongo(myclient)
    if len(data) == 0:
        return None
    else:
